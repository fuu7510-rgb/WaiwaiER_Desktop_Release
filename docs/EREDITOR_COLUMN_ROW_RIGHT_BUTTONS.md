...
### Copy（コピー）
- 表示: コピーアイコン（重なった四角形）
- ツールチップ/ラベル: `column.duplicateColumn`
- 機能: 選択中カラムを **同じテーブル内に複製**します。
  - 追加位置: 元カラムの **直下（次の順序）**
  - 名前: `{{column.name}}_copy`（重複する場合は連番で回避）
- 表示位置: **Key/Label（キー/ラベル）ボタンの左側**に、上下2段をつなげた1つのボタンとして表示
- 内部動作:
  - `duplicateColumn(tableId, column.id)`
