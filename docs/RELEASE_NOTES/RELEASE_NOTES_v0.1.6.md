...
- ERエディタ: テーブルごとにエクスポート先を選択できる機能を追加
  - テーブル設定から Excel / JSON / パッケージ(.waiwai) の出力先を個別にON/OFF可能
  - 既定は「全てにエクスポート」（既存データも自動で全てON扱い）
- AppSheet: バーチャルカラム（Virtual Column / VC）の再現
  - カラム設定で「バーチャルカラム」をON/OFF可能
  - 「Excelエクスポート時に出力されない」旨の警告ツールチップを表示
  - VCは主にAppFormulaで表示する用途（シミュレーターでは表示のみ）
  - VCはExcelエクスポート時に出力しない（ヘッダー/データとも対象外）
