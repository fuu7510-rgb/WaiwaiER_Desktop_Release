...
### ハイライト
- ERエディタ: リレーション（線）をクリックして選択・編集
  - サイドバー右ペインに「リレーション設定」パネルを追加
  - 線ラベル（自動/非表示/カスタム）、線の種類（実線/破線/点線）をリレーションごとに設定可能
  - 線アニメーション/追従アイコン表示を、デフォルト（ユーザー設定）/ON/OFF で上書き可能
- ユーザー設定: 追従アイコン設定（ON/OFF、Lucideアイコン、サイズ、速度、検索付きIconPicker）を追加
- ERエディタ: 破線/点線のアニメーションを改善（dashoffsetによる独自アニメーション）
- 内部: 互換レイヤー（スキーマ正規化）でリレーション表示設定を読み込み
...
## 変更点
### 追加
- ERエディタ: テーブルを折り畳み/展開できる機能を追加
  - 折り畳み状態はテーブル単位で保存（データスキーマ v4）
  - 折り畳み時はカラム一覧を非表示（カラム追加ボタンは利用可）
  - 折り畳み時のハンドル表示を最適化
    - 既存カラム: 従来どおり1点に集約（重ね表示）し、targetへの接続は不可
    - リターゲット対象（他テーブルから参照されているカラム）: 折り畳み時も折りたたまず表示し、接続可
  - 折り畳み時は「折り畳みテーブルがtarget」へのリレーション接続を禁止（source側は接続可）
- ユーザー設定: ショートカットキー設定機能を追加
  ...
  - ナビゲーション: 「エディタ/シミュレーター切り替え」を1項目に統合（デフォルト: Tab）
...
- ERエディタ: テーブル設定に「テーブルの説明」入力欄を追加
  - 説明はテーブル単位で保存され、各種エクスポートに含まれます（JSON/パッケージ/SQL/Markdown）
...
- エクスポート: AI開発駆動向けSQL/Markdownエクスポート機能を追加
  ...
  - テーブル説明（テーブル設定の「テーブルの説明」）をDDLコメント/Markdownに出力
...
- ERエディタ: プロジェクトを開くとクラッシュする場合がある問題を修正
  - 折り畳み時のハンドル表示（CollapsedColumnHandles）で、外部ストア購読のsnapshotが不安定になり無限再レンダーする場合がある問題を修正
